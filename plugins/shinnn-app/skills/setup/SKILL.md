---
name: setup
description: アプリの初回セットアップを対話で行う。空のフォルダでは同梱のテンプレートを展開して git と GitHub のリポジトリを用意する。テンプレートを展開した後は、テンプレートの選択・環境の検出・必須項目の確認・選択項目の決定・適用までを 1 回で通す。再実行すると選択の変更を差分の PR にする。「セットアップ」「初期設定」「最初に何をすればいい」「セットアップの選択を変えたい」で起動
---
...
- **ある**: 手順 1〜6 を順に行う。ただし再実行で、リポジトリの `.claude/rules/.standards-version` が
  ...
  標準を取り込むよう伝えて終える（規約・権限の設定・git のフック・確認のスクリプト・ワークフロー・PR テンプレートの更新は setup ではなく update-rules が扱う）
...
| husky の pre-push | 誤って `main` に push したときに断る安全網（確実に止めるのは手順 5 の 12 のブランチ保護） |
...
シン株式会社から知らされている担当者の GitHub アカウント名を聞く。
...
あわせて、画面で動かすときの開発用の通行証の入れ方は `README.md`、普段の進め方は `docs/アプリ作り方ガイド.md` に
あることを伝える。
...
- ブランチ保護のスクリプトが「使えない（理由）」と示して終わる（private で無料のプラン、管理者権限が無い）: 止めずに続け、
  理由を「有効な機能」表と `docs/decisions/` の記録に残す。`main` への push を確実に止めるのはブランチ保護なので、
  使えるプランへの変更を推奨として伝える（プランを変えるか、管理者が setup を再実行すれば設定できる）
