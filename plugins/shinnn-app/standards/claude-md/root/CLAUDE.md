...
用語: **API 定義** = 画面とサーバーがやり取りするデータの形（URL・入力・出力）を `shared` に 1 か所だけ
書いたもの。両側が同じ定義を参照するので、片方を変えるともう片方がビルドエラーになります。
`client` と `server` は互いのソースを import しません。共有するのはこの zod スキーマだけです。
...
**依存を勝手に増やさない。** 必要なら PR 本文に理由を書いて人の判断を待ちます。
