...
この変更の Issue が無いとき（`main` の上にいる、またはブランチの名前に Issue 番号が無い）は、
先に `/shinnn-app:feature` の「続きから始める」の 0 で Issue と作業ブランチを用意してから 1 に進む。
...
- **受入条件のテストが効いていること**: 受入条件（`AC-n`）を足した・変えたとき（`git diff origin/main...HEAD -- docs/仕様書.md`
  に出るとき）は、`/shinnn-app:mutation-check` で受入条件ごとに実装を壊し、そのテストが落ちることを確かめる。
...
| 確認した結果 | `/shinnn-app:check` の結果。受入条件を足した・変えたときは変異で確かめた結果（`mutation-check` の報告の 1 行） |
| **変更後の状態** | この PR がマージされると、アプリが何をできるようになるか（利用者の言葉で 2〜3 行） |
| **次の一手** | 次に作るべきもの。無ければ「無し」 |
| 規約で迷った点 | 判断に迷って仮置きした点。人の判断を待つもの |
| テストを変えない理由 | API 定義（`shared/src/api/`）を変えてテストを変えないときだけ。`テストを変えない理由: <理由>` の 1 行（`.claude/rules/api-contract.md` の「変更の手順」） |
