...
- （分かれば）シン株式会社から知らされた担当者の GitHub アカウント名。setup で聞かれます。分からなければ後から入れられます

入っているかどうかは、ターミナルで `node -v`（v24.15 以上の 24 系）・`git --version`・`gh auth status` を実行して確かめられます。
...
| 入力するもの | いつ使うか |
| :-- | :-- |
| `/shinnn-app:feature <作りたいこと>` | 作りたいこと・直したいことがあるとき（文言 1 つの直しや不具合の修正も）。いくつでもまとめて伝えてよく、分け方と順番は Claude が決めます |
...
| 有料のライブラリや、利用料のかかるサービスが必要になった | シン株式会社が判断します。Claude が「引き継ぎメモ」の「シン株式会社への要望」に残します |
