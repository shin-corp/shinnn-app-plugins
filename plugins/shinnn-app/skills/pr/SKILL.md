...
| 確認した結果 | `/shinnn-app:check` の結果。受入条件を足した・変えたとき、不具合を直したときは変異で確かめた結果（`mutation-check` の報告の 1 行）。API 定義（`shared/src/api/`）を変えてテストを変えないときは、この欄の中に `テストを変えない理由: <理由>` の 1 行を置く（見出しにしない。CI は行の頭を見る。`.claude/rules/api-contract.md` の「変更の手順」） |
